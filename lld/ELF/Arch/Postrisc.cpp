...
int64_t Postrisc::getImplicitAddend(const uint8_t *buf, RelType type) const {
  ...
  default:
    internalLinkerError(getErrorLocation(buf),
        "cannot read addend for POSTRISC relocation " + toString(type));
    return 0;
  case R_POSTRISC_32:
...
// we use NIP (next IP), not IP for ip-relative addressing
// so correct relatives forward
static uint64_t CorrectNIP(uint64_t val) {
  return val - sizeof(postrisc::CBundle);
}
...
void Postrisc::relocate(uint8_t *loc, const Relocation &rel,
  ...
  RELOC_ALL_CASES(JUMP)
    LLVM_DEBUG(dbgs() << "reloc: JUMP (" << Kind << "): " << sval
           << " offset=" << rel.offset
           << " addend=" << rel.addend << "\n");
    checkAlignment(loc, val, 4, rel);
    FixupBranch(bundle, CorrectNIP(val),
                GetSlotTypeFromKind(Kind, R_POSTRISC_A_JUMP), 0, 28, 0, 60-28);
    break;
  RELOC_ALL_CASES(BRANCH)
    LLVM_DEBUG(dbgs() << "reloc: BRANCH: " << sval << "\n");
    checkAlignment(loc, val, 4, rel);
    FixupBranch(bundle, CorrectNIP(val),
                GetSlotTypeFromKind(Kind, R_POSTRISC_A_BRANCH), 0, 17, 0, 30-17);
    break;
  RELOC_ALL_CASES(PCREL_FWD)
    LLVM_DEBUG(dbgs() << "reloc: PCREL_FWD (" << Kind << "): " << sval
           << " offset=" << rel.offset
           << " addend=" << rel.addend << "\n");
    FixupOffset(bundle, CorrectNIP(val),
                GetSlotTypeFromKind(Kind, R_POSTRISC_A_PCREL_FWD), 0, 28, 0, 64-28);
    break;
  RELOC_ALL_CASES(PCREL_FWD_GOT)
...
TargetInfo *elf::getPostriscTargetInfo()
{
  static Postrisc target;
  return &target;
}
