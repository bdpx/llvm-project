...
//
namespace {
class PostriscABIInfo : public ABIInfo {
public:
  PostriscABIInfo(CodeGenTypes &CGT) : ABIInfo(CGT) {}
...
ABIArgInfo PostriscABIInfo::classifyType(QualType Ty, unsigned SizeLimit,
                                        unsigned &RegOffset) const {
  if (Ty->isVoidType())
...
RValue PostriscABIInfo::EmitVAArg(CodeGenFunction &CGF, Address VAListAddr,
...
void PostriscABIInfo::computeInfo(CGFunctionInfo &FI) const {
...
namespace {
class PostriscTargetCodeGenInfo : public TargetCodeGenInfo {
public:
  PostriscTargetCodeGenInfo(CodeGenTypes &CGT)
      : TargetCodeGenInfo(std::make_unique<PostriscABIInfo>(CGT)) {}
...
bool
PostriscTargetCodeGenInfo::initDwarfEHRegSizeTable(CodeGen::CodeGenFunction &CGF,
...
CodeGen::createPostriscTargetCodeGenInfo(CodeGenModule &CGM) {
  return std::make_unique<PostriscTargetCodeGenInfo>(CGM.getTypes());
}
