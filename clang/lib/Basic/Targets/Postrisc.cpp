...
#include "clang/Basic/BuiltinsPostrisc.def"
};
...
ArrayRef<Builtin::Info> PostriscTargetInfo::getTargetBuiltins() const {
  return llvm::ArrayRef(BuiltinInfo, clang::Postrisc::LastTSBuiltin -
...
PostriscTargetInfo::CPUKind
PostriscTargetInfo::getCPUKind(StringRef Name) const
{
  const PostriscCPUInfo *Item = llvm::find_if(
...
void
PostriscV1TargetInfo::getTargetDefines(const LangOptions &Opts,
                                       MacroBuilder &Builder) const
{
...
void
PostriscV1TargetInfo::fillValidCPUList(SmallVectorImpl<StringRef> &Values) const
{
  for (const PostriscCPUInfo &Info : CPUInfo)
