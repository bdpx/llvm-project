...
PostriscTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  if (!CLI.IsTailCall) {
    Register RetReg = MF.addLiveIn(LinkRegister, getRegClassFor(MVT::i128));
    (void)RetReg;
  ...
  SDVTList NodeTys = DAG.getVTList(MVT::i128, MVT::Other, MVT::Glue);
  ...
  SDValue SDLinkRegister = DAG.getRegister(LinkRegister, MVT::i128);
  Chain = DAG.getCopyToReg(Chain, DL, SDLinkRegister, Call.getValue(0), InGlue);
  InGlue = Chain.getValue(1);

  Chain = DAG.getCopyFromReg(Chain, DL, LinkRegister, MVT::i128, InGlue);
  ...
  for (unsigned i = RVLocs.size(); i < n_used_regs; ++i) {
    unsigned Reg = CurrentReg;
    ++CurrentReg;
    Chain = DAG.getCopyFromReg(Chain, DL, Reg, MVT::i128/*Other*/, InGlue);
    Chain = Chain.getValue(1);
    InGlue = Chain.getValue(2);
  }
...
PostriscTargetLowering::PostriscTargetLowering(const TargetMachine &TM,
  ...
  setOperationAction({ ISD::ADD, ISD::SUB,
                       ISD::SDIV, //ISD::SREM,
                       ISD::UDIV, //ISD::UREM,
                       ISD::SHL, ISD::SRA, ISD::SRL }, MVT::i128, Legal);

  // FIXME: this is a workaround to disallow use i128 mul for i64 div
  setOperationAction({ISD::MUL, ISD::UMULO, ISD::SMULO}, MVT::i128, LibCall);
  ...
  setOperationAction(ISD::IS_FPCLASS, { MVT::f16, MVT::f32, MVT::f64, MVT::f128 }, Legal);
  ...
  setOperationAction(ISD::SETCC, { MVT::f16, MVT::f32, MVT::f64, MVT::f128 }, Legal); //Expand?
  ...
  for (auto VT : { MVT::i8, MVT::i16, MVT::i32, MVT::i64, MVT::i128 }) {
    setOperationAction({
       ...
       ISD::ATOMIC_SWAP, ISD::ATOMIC_CMP_SWAP
       }, VT, Legal);
  ...
  // FPU
  for (auto VT : { MVT::f16, MVT::f32, MVT::f64, MVT::f128 }) {
    setOperationAction(ISD::FGETSIGN, VT, Legal); // FIXME: clang never produces fgetsign
  }

  for (auto VT : { MVT::f16, MVT::f32, MVT::f64, MVT::f128, MVT::v8f16, MVT::v4f32, MVT::v2f64 }) {
    setOperationAction({
      ISD::FSIN, ISD::FCOS, ISD::FSINCOS,
      ISD::FREM, ISD::FPOW
      }, VT, Expand);
    ...
    setOperationAction({
      ...
      ISD::FMINNUM, ISD::STRICT_FMINNUM,
      ISD::FMAXNUM, ISD::STRICT_FMAXNUM,
      ISD::FMINIMUM, ISD::STRICT_FMINIMUM,
      ISD::FMAXIMUM, ISD::STRICT_FMAXIMUM,
      ...
      ISD::LRINT, ISD::STRICT_LRINT,
      ISD::LLRINT, ISD::STRICT_LLRINT,
      ISD::LROUND, ISD::STRICT_LROUND,
      ISD::LLROUND, ISD::STRICT_LLROUND,
      }, VT, Legal);
  ...
  for (auto VT : { MVT::v16i8, MVT::v8i16, MVT::v4i32, MVT::v2i64 }) {
    ...
    // __builtin_elementwise_[min,max,add_sat,sub_sat]
    setOperationAction({ ISD::SMIN, ISD::SMAX, ISD::UMIN, ISD::UMAX,
                         ISD::AVGCEILS, ISD::AVGCEILU,
                         ISD::ROTL, ISD::ROTR, ISD::FSHL, ISD::FSHR, ISD::VSELECT,
...
bool PostriscTargetLowering::isFPImmLegal(const APFloat &Imm, EVT VT,
  ...
  if (VT == MVT::f16 || VT == MVT::f32 || VT == MVT::f64)
    return true; // LDI/LDI_EXT
  if (VT == MVT::f128)
    return true; // LDIH/LDIH_EXT + LDI/LDI_EXT
...
bool PostriscTargetLowering::shouldInsertFencesForAtomic(const Instruction *I) const {
  return false;
#if 0
  // insert fences if we not support seq_cst in hardware
