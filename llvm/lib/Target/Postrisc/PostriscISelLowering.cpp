...
// Maybe this could be a TableGen attribute on some registers and
...
PostriscTargetLowering::PostriscTargetLowering(const TargetMachine &TM,
  ...
  setOperationAction({ISD::FSHL, ISD::FSHR}, MVT::i64, Legal);
  setOperationAction({ISD::FSHL, ISD::FSHR}, MVT::i128, Legal);
  ...
  for (auto VT : { MVT::i32, MVT::i64 }) {
    ...
    setOperationAction({ISD::SREM, ISD::UREM}, VT, Legal);
...
SDValue PostriscTargetLowering::makeAddress(SDValue Op, SelectionDAG &DAG, bool isCode) const {
  ...
  if (isPositionIndependent()) {
    if (IsCodeModelExtInsnRequired(isCode)) {
      ...
      TF = isCode ? PostriscMCExpr::VK_JUMP_EXT
                  : PostriscMCExpr::VK_PCREL_FWD_EXT;
      ...
      TF = isCode ? PostriscMCExpr::VK_JUMP
                  : PostriscMCExpr::VK_PCREL_FWD;
  ...
  case CodeModel::Medium: {
    return DAG.getNode(ISD::ADD, DL, PtrVT,
               ...
               withTargetFlags(Op, PostriscMCExpr::VK_OFFSET, DAG));
  }
  case CodeModel::Large: {
    return DAG.getNode(ISD::ADD, DL, PtrVT,
               DAG.getRegister(POSTRISC::gz, PtrVT),
               withTargetFlags(Op, PostriscMCExpr::VK_OFFSET_EXT, DAG));
  }
  }
}
...
SDValue PostriscTargetLowering::LowerGlobalTLSAddress(SDValue Op,
  ...
  if (model == TLSModel::GeneralDynamic || model == TLSModel::LocalDynamic) {
    ...
    unsigned const GotTF = isGeneral
      ? (isExtData ? PostriscMCExpr::VK_TLS_GD_GOT_OFFSET_EXT
                   : PostriscMCExpr::VK_TLS_GD_GOT_OFFSET)
      : (isExtData ? PostriscMCExpr::VK_TLS_LD_GOT_OFFSET_EXT
                   : PostriscMCExpr::VK_TLS_LD_GOT_OFFSET);
    unsigned const callTF = isGeneral
      ? (isExtCode ? PostriscMCExpr::VK_TLS_GD_CALL_EXT
                   : PostriscMCExpr::VK_TLS_GD_CALL)
      : (isExtCode ? PostriscMCExpr::VK_TLS_LD_CALL_EXT
                   : PostriscMCExpr::VK_TLS_LD_CALL);
    ...
    SDValue Callee = DAG.getTargetExternalSymbol("__tls_get_addr", PtrVT,
                         isExtCode ? PostriscMCExpr::VK_JUMP_EXT
                                   : PostriscMCExpr::VK_JUMP);
    ...
    //
    unsigned const TF = isExtData ? PostriscMCExpr::VK_TLS_LD_OFFSET_EXT
                                  : PostriscMCExpr::VK_TLS_LD_OFFSET;
  ...
  if (model == TLSModel::InitialExec) {
    ...
    //
    unsigned const TF = isExtData ? PostriscMCExpr::VK_TLS_IE_OFFSET_EXT
                                  : PostriscMCExpr::VK_TLS_IE_OFFSET;
  ...
  //
  unsigned const TF = isExtData ? PostriscMCExpr::VK_TLS_LE_OFFSET_EXT
                                : PostriscMCExpr::VK_TLS_LE_OFFSET;
...
PostriscTargetLowering::LowerCustomJumpTableEntry(const MachineJumpTableInfo *MJTI,
  ...
  // entries.
  return MCSymbolRefExpr::create(MBB->getSymbol(), PostriscMCExpr::VK_GOTOFF, Ctx);
}
