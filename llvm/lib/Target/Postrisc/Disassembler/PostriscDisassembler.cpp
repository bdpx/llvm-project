...
// TODO: should be synced with encoding, find a way to use autogenerated
static const unsigned SRDecoderTable[128] = {
...
static DecodeStatus DecodeSRRegisterClass(MCInst &Inst, unsigned RegNo,
                                              uint64_t Address,
                                              const MCDisassembler *Decoder) {
  if (RegNo >= std::size(SRDecoderTable))
    return MCDisassembler::Fail;
  if (SRDecoderTable[RegNo] == RESERVED)
    return MCDisassembler::Fail;
  Inst.addOperand(MCOperand::createReg(SRDecoderTable[RegNo]));
  return MCDisassembler::Success;
}
