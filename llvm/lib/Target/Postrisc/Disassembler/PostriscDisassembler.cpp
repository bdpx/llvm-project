...
static DecodeStatus decodeFRMArg(MCInst &Inst, uint32_t Imm, int64_t Address, const MCDisassembler *Decoder) {
  ...
  if (!llvm::PostriscFPRndMode::isValidRoundingMode(Imm))
    return MCDisassembler::Fail;
...
static DecodeStatus decodeMemoryOrderingArg(MCInst &Inst, uint32_t Imm, int64_t Address, const MCDisassembler *Decoder) {
  assert(isUInt<3>(Imm) && "Invalid immediate");
  if (!llvm::PostriscMemoryOrdering::isValidMemoryOrdering(Imm))
    return MCDisassembler::Fail;
