...
class PostriscTargetMachine : public LLVMTargetMachine {
...
class PostriscV2TargetMachine : public PostriscTargetMachine {
  virtual void anchor();

public:
  PostriscV2TargetMachine(const Target &T, const Triple &TT, StringRef CPU,
                       StringRef FS, const TargetOptions &Options,
                       std::optional<Reloc::Model> RM,
                       std::optional<CodeModel::Model> CM, CodeGenOptLevel OL,
                       bool JIT);
