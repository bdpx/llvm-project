...
void PostriscMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  const llvm::endianness endianness =
        Ctx.getAsmInfo().isLittleEndian() ? llvm::endianness::little : llvm::endianness::big;
  ...
  for (auto &I : Postrisc::bundleInstructions(MI)) {
    ...
    const unsigned Opcode = HMI.getOpcode();
    const MCInstrDesc &Desc = MCII.get(Opcode);
    const bool isLong = PostriscII::isLong(Desc.TSFlags);
