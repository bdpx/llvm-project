...
static MCRegisterInfo *createPostriscMCRegisterInfo(const Triple &TT) {
  ...
  InitPostriscMCRegisterInfo(X, POSTRISC::r0); //FIXME - link register?
  return X;
}
...
createPostriscMCSubtargetInfo(const Triple &TT, StringRef CPU, StringRef FS) {
  if (CPU.empty())
    CPU = "postrisc";
  return createPostriscMCSubtargetInfoImpl(TT, CPU, /*TuneCPU*/ CPU, FS);
}

static MCTargetStreamer *
createObjectTargetStreamer(MCStreamer &S, const MCSubtargetInfo &STI) {
  return new PostriscTargetELFStreamer(S);
}
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializePostriscTargetMC() {
  // Register the MC asm info.
  RegisterMCAsmInfoFn X(getThePostriscTarget(), createPostriscMCAsmInfo);

  for (Target *T : {&getThePostriscTarget()}) {
