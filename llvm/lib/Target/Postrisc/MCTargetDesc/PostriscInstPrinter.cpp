...
void PostriscInstPrinter::printFRMArg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  auto FRMArg = static_cast<llvm::postrisc::RoundingMode>(MI->getOperand(OpNo).getImm());
  //if (FRMArg == llvm::postrisc::RoundingMode::DYN)
  //    return;
  O << llvm::postrisc::roundingModeToString(FRMArg);
}

void PostriscInstPrinter::printMemoryOrderingArg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  auto FRMArg = static_cast<llvm::postrisc::MemoryOrdering>(MI->getOperand(OpNo).getImm());
  //if (FRMArg == llvm::postrisc::RELAXED)
  //    return;
  O << llvm::postrisc::memoryOrderingToString(FRMArg);
}
