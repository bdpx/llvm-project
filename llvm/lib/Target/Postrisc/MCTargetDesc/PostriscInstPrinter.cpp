...
void PostriscInstPrinter::printFRMArg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  auto FRMArg = static_cast<PostriscFPRndMode::RoundingMode>(MI->getOperand(OpNo).getImm());
  //if (FRMArg == PostriscFPRndMode::RoundingMode::DYN)
  //    return;
  O << PostriscFPRndMode::roundingModeToString(FRMArg);
}

void PostriscInstPrinter::printMemoryOrderingArg(const MCInst *MI, unsigned OpNo,
                                   const MCSubtargetInfo &STI, raw_ostream &O) {
  auto FRMArg = static_cast<PostriscMemoryOrdering::MemoryOrdering>(MI->getOperand(OpNo).getImm());
  //if (FRMArg == PostriscMemoryOrdering::MemoryOrdering::RELAXED)
  //    return;
  O << PostriscMemoryOrdering::memoryOrderingToString(FRMArg);
}
