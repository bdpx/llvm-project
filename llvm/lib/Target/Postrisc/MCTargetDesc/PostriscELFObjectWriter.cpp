...
unsigned PostriscELFObjectWriter::getRelocType(const MCFixup &Fixup,
  ...
  case PostriscMCExpr::VK_TLS_GD_GOT_OFFSET:
  case PostriscMCExpr::VK_TLS_LD_GOT_OFFSET:
  case PostriscMCExpr::VK_TLS_LD_OFFSET:
  case PostriscMCExpr::VK_TLS_IE_OFFSET:
  case PostriscMCExpr::VK_TLS_LE_OFFSET:
  ...
  if (const PostriscMCExpr *SExpr = dyn_cast<PostriscMCExpr>(Fixup.getValue())) {
    if (SExpr->getSpecifier() == PostriscMCExpr::VK_R_DISP32)
      return ELF::R_POSTRISC_DISP32;
  }

  switch(Fixup.getTargetKind()) {
  default:
    LLVM_DEBUG(dbgs() << "Fixup.getTargetKind()=" << Fixup.getTargetKind() << "\n");
