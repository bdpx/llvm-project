...
void PostriscAsmPrinter::printOperand(const MachineInstr *MI, int opNum,
  ...
  PostriscMCExpr::VariantKind TF = (PostriscMCExpr::VariantKind) MO.getTargetFlags();

#if !defined(NDEBUG) && 0
  // FIXME: Verify the target flags.
  if (MO.isGlobal() || MO.isSymbol() || MO.isCPI()) {
    if (MI->getOpcode() == POSTRISC::CALL)
      assert(TF == PostriscMCExpr::VK_Postrisc_NONE &&
             "Cannot handle target flags on call address");
    else if (MI->getOpcode() == POSTRISC::TLS_CALL)
      assert((TF == PostriscMCExpr::VK_Postrisc_NONE
              || TF == PostriscMCExpr::VK_Postrisc_TLS_GD_CALL
              || TF == PostriscMCExpr::VK_Postrisc_TLS_LD_CALL) &&
             "Cannot handle target flags on tls call address");
    else
      assert((TF == PostriscMCExpr::VK_Postrisc_TLS_GD_GOT_OFFSET
              || TF == PostriscMCExpr::VK_Postrisc_TLS_LD_GOT_OFFSET
              || TF == PostriscMCExpr::VK_Postrisc_TLS_IE_OFFSET ) &&
             "Invalid target flags for small address operand");
  }
#endif


  bool CloseParen = PostriscMCExpr::printVariantKind(O, TF);
