...
static cl::opt<bool> EnablePrePostUpdate("postrisc-enable-pre-post-update",
...
#define X(OP, OP_MIA) case POSTRISC::OP: return POSTRISC::OP_MIA;
      X(LDUB, LDUBMIA) X(LDUH, LDUHMIA) X(LDUW, LDUWMIA) X(LDUD, LDUDMIA)
      X(LDSB, LDSBMIA) X(LDSH, LDSHMIA) X(LDSW, LDSWMIA) X(LDSD, LDSDMIA) X(LDQ, LDQMIA)
      X(STB, STBMIA) X(STH, STHMIA) X(STW, STWMIA) X(STD, STDMIA) X(STQ, STQMIA)
#undef X
    }
    return -1;
  }
...
#define X(OP, OP_MIB) case POSTRISC::OP: return POSTRISC::OP_MIB;
      X(LDUB, LDUBMIB) X(LDUH, LDUHMIB) X(LDUW, LDUWMIB) X(LDUD, LDUDMIB)
      X(LDSB, LDSBMIB) X(LDSH, LDSHMIB) X(LDSW, LDSWMIB) X(LDSD, LDSDMIB) X(LDQ, LDQMIB)
      X(STB, STBMIB) X(STH, STHMIB) X(STW, STWMIB) X(STD, STDMIB) X(STQ, STQMIB)
#undef X
    }
    return -1;
  }
...
private:
  ...
  bool canCombine(MachineInstr &Ldst, Register B);
  ...
  MachineInstr *canJoinInstructions(MachineInstr *Ldst, MachineInstr *Add,
                                    SmallVectorImpl<MachineInstr *> *Uses, bool& postIncrement);
...
bool PostriscOptAddrMode::canCombine(MachineInstr &Ldst, Register B)
{
  bool isAddFound = false;
  for (const MachineInstr &Add : MRI->use_nodbg_instructions(B)) {
    LLVM_DEBUG(dbgs() << "canCombine? " << Add);
    int64_t Incr;
    ...
    if (&Add == &Ldst)
      continue;
    LLVM_DEBUG(dbgs() << "canCombine: no\n");
    return false;
  }
  LLVM_DEBUG(dbgs() << "canCombine(final): " << (isAddFound ? "yes" : "no") << '\n');
  return isAddFound;
}
...
MachineInstr *PostriscOptAddrMode::tryToCombine(MachineInstr &Ldst) {
  ...
  if (!canCombine(Ldst, B)) {
    return nullptr;
  }

  for (MachineInstr &Add : MRI->use_nodbg_instructions(B)) {
    int64_t Incr;
    if (!isAddConstantOp(Add, Incr))
      continue;
    ...
    bool postIncrement = true;
    MachineInstr *MoveTo = canJoinInstructions(&Ldst, &Add, &Uses, postIncrement);
    ...
    int NewOpcode = postIncrement ? Postrisc::getPostIncOpcode(Ldst.getOpcode()) : Postrisc::getPreIncOpcode(Ldst.getOpcode());
