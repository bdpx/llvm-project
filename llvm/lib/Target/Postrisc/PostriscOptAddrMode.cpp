...
namespace Postrisc {
  int getPostIncOpcode(int opcode) {
    switch (opcode) {
#define X(OP) case POSTRISC::OP: return POSTRISC::OP ## MIA;
      X(LDBZ) X(LDHZ) X(LDWZ) X(LDDZ) X(LDQ)
      X(LDBS) X(LDHS) X(LDWS) X(LDDS)
...
#undef X
  ...
  int getPreIncOpcode(int opcode) {
    switch (opcode) {
#define X(OP) case POSTRISC::OP: return POSTRISC::OP ## MIB;
      X(LDBZ) X(LDHZ) X(LDWZ) X(LDDZ) X(LDQ)
      X(LDBS) X(LDHS) X(LDWS) X(LDDS)
      X(STB) X(STH) X(STW) X(STD)
#undef X
...
static bool isValidIncrementOffset(int64_t Off) { return isInt<10>(Off); }
