...
void PostriscFrameLowering::emitSPAdjustment(MachineFunction &MF,
  ...
  // addi sp, sp, NumBytes
  unsigned const Opcode = isInt<21>(NumBytes) ? POSTRISC::ADDri : POSTRISC::ADDri_ext;
  assert(isInt<21+42>(NumBytes));
...
void PostriscFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (!hasFP(MF) && NumBytes == 0) { // FuncInfo->isLeafProc()
  ...
  unsigned const Opcode = isUInt<21>(NumBytes) ? POSTRISC::ALLOCSP
                                               : POSTRISC::ALLOCSP_ext;
  ...
  if (hasFP(MF)) {
    ...
    assert((NewI->getOpcode() == POSTRISC::ADDri || NewI->getOpcode() == POSTRISC::ADDri_ext)
  ...
  if (NeedsStackRealignment) {
    ...
    if (Bias) {
      ...
      // add %o6, BIAS, %g1
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDri), regUnbiased)
    ...
    unsigned const Opcode =
      isInt<21>(MaxAlign.value() - 1U) ? POSTRISC::ANDNri
                                       : POSTRISC::ANDNri_ext;
    BuildMI(MBB, MBBI, dl, TII.get(Opcode), POSTRISC::sp)
        .addReg(POSTRISC::sp)
    ...
    if (Bias) {
      // add %g1, -BIAS, %o6
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDri), POSTRISC::sp)
...
void PostriscFrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (hasFP(MF)) {
    // we do hasFP save/restore SP/FP in PostriscFrameFixer
    return;
  }
  ...
  if (!isUInt<21>(NumBytes)) {
    assert(isUInt<63>(NumBytes));
...
bool PostriscFrameLowering::hasFP(const MachineFunction &MF) const {
...
{
#if 0
  for (unsigned reg = POSTRISC::I0; reg <= POSTRISC::I7; ++reg)
    if (MRI->isPhysRegUsed(reg))
      return false;

  for (unsigned reg = POSTRISC::L0; reg <= POSTRISC::L7; ++reg)
    if (MRI->isPhysRegUsed(reg))
      return false;
#endif
...
// unused
void PostriscFrameLowering::remapRegsForLeafProc(MachineFunction &MF) const {
  MachineRegisterInfo &MRI = MF.getRegInfo();
  const int offset = 15; //FIXME
  for (unsigned reg = POSTRISC::r95; reg <= POSTRISC::r111; ++reg) {
    if (!MRI.isPhysRegUsed(reg))
      continue;
    unsigned const mapped_reg = reg + offset;
