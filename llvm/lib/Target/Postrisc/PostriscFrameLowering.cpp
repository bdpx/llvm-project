...
static cl::opt<bool>
DisableLeafProc("postrisc-disable-leaf-proc",
                cl::init(false),
...
void PostriscFrameLowering::emitSPAdjustment(MachineFunction &MF,
  ...
  // addi sp, sp, NumBytes
  unsigned const Opcode = isInt<llvm::postrisc::InstructionFieldSize::BinaryImm>(NumBytes) ? POSTRISC::ADDID : POSTRISC::ADDID_EXT;
...
void PostriscFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  if (hasFPImpl(MF)) {
    ...
    assert((NewI->getOpcode() == POSTRISC::ADDID || NewI->getOpcode() == POSTRISC::ADDID_EXT)
  ...
  if (NeedsStackRealignment) {
    ...
    if (Bias) {
      ...
      // add %o6, BIAS, %g1
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDID), regUnbiased)
    ...
    unsigned const Opcode = isInt<llvm::postrisc::InstructionFieldSize::BinaryImm>(MaxAlign.value() - 1U) ? POSTRISC::ANDNI : POSTRISC::ANDNI_EXT;
    BuildMI(MBB, MBBI, dl, TII.get(Opcode), POSTRISC::sp)
        .addReg(POSTRISC::sp)
    ...
    if (Bias) {
      // add %g1, -BIAS, %o6
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDID), POSTRISC::sp)
