...
void PostriscFrameLowering::emitSPAdjustment(MachineFunction &MF,
  ...
  // addi sp, sp, NumBytes
  unsigned const Opcode = isInt<21>(NumBytes) ? POSTRISC::ADDI : POSTRISC::ADDI_EXT;
...
void PostriscFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  unsigned const Opcode = isUInt<21>(NumBytes) ? POSTRISC::ALLOCSP : POSTRISC::ALLOCSP_EXT;
  ...
  if (hasFPImpl(MF)) {
    ...
    assert((NewI->getOpcode() == POSTRISC::ADDI || NewI->getOpcode() == POSTRISC::ADDI_EXT)
  ...
  if (NeedsStackRealignment) {
    ...
    if (Bias) {
      ...
      // add %o6, BIAS, %g1
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDI), regUnbiased)
    ...
    unsigned const Opcode = isInt<21>(MaxAlign.value() - 1U) ? POSTRISC::ANDNI : POSTRISC::ANDNI_EXT;
    BuildMI(MBB, MBBI, dl, TII.get(Opcode), POSTRISC::sp)
        .addReg(POSTRISC::sp)
    ...
    if (Bias) {
      // add %g1, -BIAS, %o6
      BuildMI(MBB, MBBI, dl, TII.get(POSTRISC::ADDI), POSTRISC::sp)
