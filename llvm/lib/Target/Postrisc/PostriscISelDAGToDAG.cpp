...
public:
  ...
  SDValue getI32Imm(int32_t Imm, const SDLoc &DL) {
    return CurDAG->getSignedTargetConstant(Imm, DL, MVT::i32);
  }
  SDValue getI64Imm(int64_t Imm, const SDLoc &DL) {
    return CurDAG->getSignedTargetConstant(Imm, DL, MVT::i64);
  }
  SDValue getU32Imm(uint32_t Imm, const SDLoc &DL) {
    return CurDAG->getTargetConstant(Imm, DL, MVT::i32);
  }
  SDValue getU64Imm(uint64_t Imm, const SDLoc &DL) {
    return CurDAG->getTargetConstant(Imm, DL, MVT::i64);
  }

  bool isSextInt(const APInt &Imm, unsigned nbits) const {
      return Imm.getSignificantBits() <= nbits; // APInt signed len
  }
  bool isZextInt(const APInt &Imm, unsigned nbits) const {
