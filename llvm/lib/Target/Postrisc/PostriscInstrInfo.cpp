...
Register PostriscInstrInfo::isLoadFromStackSlot(const MachineInstr &MI,
  ...
  if (MI.getOpcode() == POSTRISC::LDWZ ||
      MI.getOpcode() == POSTRISC::LDWS ||
      MI.getOpcode() == POSTRISC::LDDZ ||
      MI.getOpcode() == POSTRISC::LDDS ||
      MI.getOpcode() == POSTRISC::LDQ) {
...
Register PostriscInstrInfo::isStoreToStackSlot(const MachineInstr &MI,
                                            int &FrameIndex) const {
  if (MI.getOpcode() == POSTRISC::STW ||
      MI.getOpcode() == POSTRISC::STD ||
      MI.getOpcode() == POSTRISC::STQ) {
    if (MI.getOperand(1).isFI() &&
        MI.getOperand(2).isImm() &&
        MI.getOperand(2).getImm() == 0) {
      FrameIndex = MI.getOperand(1).getIndex();
...
#define DECLARE_INVERTED_WITHOUT_ARG_SWAP_COND_BRANCHES(X) \
  X(BDEQ, BDNE) X(BDEQI, BDNEI) X(BDLTI, BDGEI) X(BDLTUI, BDGEUI) \
  X(BWEQ, BWNE) X(BWEQI, BWNEI) X(BWLTI, BWGEI) X(BWLTUI, BWGEUI) \
  X(BBS, BBC) X(BBSI, BBCI) \
  X(BMALL, BMNOTALL) X(BMNONE, BMANY) \
  X(BFSSOEQ, BFSSUNE) X(BFSSONE, BFSSUEQ) X(BFSSO, BFSSUO) \
  X(BFSDOEQ, BFSDUNE) X(BFSDONE, BFSDUEQ) X(BFSDO, BFSDUO) \
  X(BFSQOEQ, BFSQUNE) X(BFSQONE, BFSQUEQ) X(BFSQO, BFSQUO) \

#define DECLARE_INVERTED_WITH_ARG_SWAP_COND_BRANCHES(X) \
  X(BDLT, BDLE) X(BDLTU, BDLEU) X(BWLT, BWLE) X(BWLTU, BWLEU) /* lt  invert=> ge  swap=> le  */ \
  X(BFSSOLT, BFSSULE) X(BFSDOLT, BFSDULE) X(BFSQOLT, BFSQULE) /* olt invert=> uge swap=> ule */ \
  X(BFSSOLE, BFSSULT) X(BFSDOLE, BFSDULT) X(BFSQOLE, BFSQULT) /* ole invert=> ugt swap=> ult */ \

#define DECLARE_COND_BRANCHES(X) \
  DECLARE_INVERTED_WITH_ARG_SWAP_COND_BRANCHES(X) DECLARE_INVERTED_WITHOUT_ARG_SWAP_COND_BRANCHES(X)
...
bool PostriscInstrInfo::isBranchOffsetInRange(unsigned BranchOpc,
  ...
  switch (BranchOpc) {
  case POSTRISC::JMP:        return isIntN((int)llvm::postrisc::InstructionFieldSize::CallDisp, Offset >> 4);
  case POSTRISC::JMP_EXT:    return isIntN((int)llvm::postrisc::LongInstructionFieldSize::CallDisp, Offset >> 4);
...
void PostriscInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (POSTRISC::GRRegClass.contains(DestReg, SrcReg)) {
    ...
    BuildMI(MBB, I, DL, get(POSTRISC::MTSPR), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  } else if (POSTRISC::SRRegClass.contains(SrcReg) && POSTRISC::GRRegClass.contains(DestReg)) {
    BuildMI(MBB, I, DL, get(POSTRISC::MFSPR), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  } else {
...
void PostriscInstrInfo::storeRegToStackSlot(MachineBasicBlock &MBB,
  ...
  BuildMI(MBB, I, DL, get(POSTRISC::STQ))
...
void PostriscInstrInfo::loadRegFromStackSlot(
  ...
  assert(RC == &POSTRISC::GRRegClass);
  BuildMI(MBB, I, DL, get(POSTRISC::LDQ))
