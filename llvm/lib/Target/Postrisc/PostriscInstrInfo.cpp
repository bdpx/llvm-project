...
{
  return PostriscII::isLong(get(Opc).TSFlags);
}

bool PostriscInstrInfo::isPostIncrement(const MachineInstr &MI) const {
  return PostriscII::isPostIncrement(MI.getDesc().TSFlags);
}

bool PostriscInstrInfo::isPreIncrement(const MachineInstr &MI) const {
  return PostriscII::isPreIncrement(MI.getDesc().TSFlags);
}

bool PostriscInstrInfo::isBaseMemAccess(const MachineInstr &MI) const {
  return PostriscII::isBaseMemAccess(MI.getDesc().TSFlags);
}
...
#define DECLARE_COND_BRANCHES(X) \
  ...
  X(BRBS,      BRBC) \
  X(BRBSI,     BRBCI) \
  X(BRMALLW,   BRMNOTALLW) \
  X(BRMNONEW,  BRMANYW) \
  X(BRMALLD,   BRMNOTALLD) \
  X(BRMNONED,  BRMANYD) \
  X(BRMALLQ,   BRMNOTALLQ) \
  X(BRMNONEQ,  BRMANYQ) \
  X(FBROEQH,   FBRUNEH) \
  X(FBRONEH,   FBRUEQH) \
  X(FBROLTH,   FBRUGEH) \
...
void PostriscInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (POSTRISC::GRRegClass.contains(DestReg, SrcReg)) {
    ...
    BuildMI(MBB, I, DL, get(POSTRISC::SETSPR), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  } else if (POSTRISC::SRRegClass.contains(SrcReg) && POSTRISC::GRRegClass.contains(DestReg)) {
    BuildMI(MBB, I, DL, get(POSTRISC::GETSPR), DestReg)
      .addReg(SrcReg, getKillRegState(KillSrc));
  } else {
...
unsigned PostriscInstrInfo::getInstSizeInBytes(const MachineInstr &MI) const {
