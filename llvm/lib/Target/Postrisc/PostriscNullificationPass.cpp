...
bool PostriscNullification::Convert(MachineBasicBlock *Head,
  ...
  if (swap_blocks) {
    if (TBB != Tail) {
      LLVM_DEBUG(dbgs() << "\nTBB (merged 1st): " << TBB->getParent()->getName() << ":\n"; TBB->dump());
      Head->splice(IP, TBB, TBB->begin(), TBB->getFirstTerminator());
    }
    if (FBB != Tail) {
      LLVM_DEBUG(dbgs() << "\nFBB (merged 2nd): " << FBB->getParent()->getName() << "\n"; FBB->dump());
      Head->splice(IP, FBB, FBB->begin(), FBB->getFirstTerminator());
    }
  } else {
    if (FBB != Tail) {
      LLVM_DEBUG(dbgs() << "\nFBB (merged 1st): " << FBB->getParent()->getName() << "\n"; FBB->dump());
      Head->splice(IP, FBB, FBB->begin(), FBB->getFirstTerminator());
    }
    if (TBB != Tail) {
      LLVM_DEBUG(dbgs() << "\nTBB (merged 2nd): " << TBB->getParent()->getName() << "\n"; TBB->dump());
      Head->splice(IP, TBB, TBB->begin(), TBB->getFirstTerminator());
    }
  }
  ...
  LLVM_DEBUG(dbgs() << "\nHead (merged): " << Head->getParent()->getName() << "\n"; Head->dump());
  ...
  if (!ExtraPreds && Head->isLayoutSuccessor(Tail)) {
    ...
    LLVM_DEBUG(dbgs() << "\nHead (tailed): " << Head->getParent()->getName() << "\n"; Head->dump());
  } else {
    ...
    LLVM_DEBUG(dbgs() << "\nHead (final): " << Head->getParent()->getName() << "\n"; Head->dump());
  }
