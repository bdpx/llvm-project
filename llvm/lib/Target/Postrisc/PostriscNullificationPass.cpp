...
#define X(COND)                                                                                       \
  case POSTRISC::B##COND:        swap_args = false; swap_blocks = false; return POSTRISC::NUL##COND;  \
  case POSTRISC::B##COND##_EXT:  swap_args = false; swap_blocks = false; return POSTRISC::NUL##COND;

  X(DEQ) X(DNE) X(DLT) X(DLE) X(DLTU) X(DLEU)
  X(WEQ) X(WNE) X(WLT) X(WLE) X(WLTU) X(WLEU)
  X(BS) X(BC) X(BSI) X(BCI)
  X(FSSOEQ) X(FSSONE) X(FSSOLT) X(FSSOLE) X(FSSO) X(FSSUEQ) X(FSSUNE) X(FSSULT) X(FSSULE) X(FSSUO)
  X(FSDOEQ) X(FSDONE) X(FSDOLT) X(FSDOLE) X(FSDO) X(FSDUEQ) X(FSDUNE) X(FSDULT) X(FSDULE) X(FSDUO)
  X(FSQOEQ) X(FSQONE) X(FSQOLT) X(FSQOLE) X(FSQO) X(FSQUEQ) X(FSQUNE) X(FSQULT) X(FSQULE) X(FSQUO)

// XX - short/long branch reg-imm branches convert to corresponding short/long nullifications
#define XX(COND)                                                                                              \
  case POSTRISC::B##COND:        swap_args = false; swap_blocks = false; return POSTRISC::NUL##COND;          \
  case POSTRISC::B##COND##_EXT:  swap_args = false; swap_blocks = false; return POSTRISC::NUL##COND##_EXT;
...
#define XXSWAP(BCOND, NCOND)                                                                                   \
  case POSTRISC::B##BCOND:        swap_args = false; swap_blocks = true; return POSTRISC::NUL##NCOND;          \
  case POSTRISC::B##BCOND##_EXT:  swap_args = false; swap_blocks = true; return POSTRISC::NUL##NCOND##_EXT;
