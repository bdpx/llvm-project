...
#include "PostriscGenAsmMatcher.inc"
  ...
  const PostriscMCExpr *adjustPICRelocation(PostriscMCExpr::Specifier VK,
...
PostriscAsmParser::parsePostriscAsmOperand(std::unique_ptr<PostriscOperand> &Op,
  ...
  case AsmToken::Identifier: {
    ...
    if (!getParser().parseIdentifier(Identifier)) {
      ...
      PostriscMCExpr::Specifier Kind = PostriscMCExpr::VK_NONE;

      if (getContext().getObjectFileInfo()->isPositionIndependent()) {
        if (isCall)
          Kind = PostriscMCExpr::VK_JUMP;
        else
          Kind = PostriscMCExpr::VK_PCREL_FWD;
      }
...
PostriscAsmParser::adjustPICRelocation(PostriscMCExpr::Specifier VK,
                                    const MCExpr *subExpr) {
...
bool PostriscAsmParser::matchPostriscAsmModifiers(const MCExpr *&EVal,
  ...
  PostriscMCExpr::Specifier VK = PostriscMCExpr::parseSpecifier(name);
  switch (VK) {
  case PostriscMCExpr::VK_NONE:
    Error(getLoc(), "invalid operand modifier");
    return false;

  case PostriscMCExpr::VK_TLS_GD_CALL:
