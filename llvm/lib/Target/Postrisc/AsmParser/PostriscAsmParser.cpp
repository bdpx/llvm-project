...
private:
  ...
  struct FRMOp {
    llvm::postrisc::RoundingMode FRM;
  };

  struct MemoryOrderingOp {
    llvm::postrisc::MemoryOrdering  MO;
  };
...
public:
  ...
  llvm::postrisc::RoundingMode getFRM() const {
    assert(Kind == k_FRM && "Invalid type access!");
    return FRM.FRM;
  }

  llvm::postrisc::MemoryOrdering getMemoryOrdering() const {
  ...
  void print(raw_ostream &OS, const MCAsmInfo &MAI) const override {
    ...
    case k_FRM:       OS << "frm: " << llvm::postrisc::roundingModeToString(getFRM()) << "\n";  break;
  ...
  createFRMArg(llvm::postrisc::RoundingMode FRM, SMLoc S) {
  ...
  static std::unique_ptr<PostriscOperand>
  createMemoryOrderingArg(llvm::postrisc::MemoryOrdering MO, SMLoc S) {
...
bool PostriscAsmParser::matchAndEmitInstruction(SMLoc IDLoc, unsigned &Opcode,
  ...
  if (PostriscII::isLong(Desc.TSFlags)) {
    bundle_size += 2;
  } else {
    bundle_size += 1;
  }
...
ParseStatus PostriscAsmParser::parseFRMArg(OperandVector &Operands) {
  ...
  const llvm::postrisc::RoundingMode FRM = llvm::postrisc::stringToRoundingMode(Str);

  if (FRM == llvm::postrisc::RM_INVALID)
    return TokError(
        "operand must be a valid floating point rounding mode mnemonic");
...
ParseStatus PostriscAsmParser::parseMemoryOrderingArg(OperandVector &Operands) {
  ...
  StringRef Str = getLexer().getTok().getIdentifier();
  const llvm::postrisc::MemoryOrdering MO = llvm::postrisc::stringToMemoryOrdering(Str);

  if (MO == llvm::postrisc::MO_INVALID)
    return TokError(
        "operand must be a valid memory ordering mnemonic");
...
std::unique_ptr<PostriscOperand> PostriscAsmParser::defaultFRMArgOp() const {
  return PostriscOperand::createFRMArg(llvm::postrisc::RM_DYN,
                                    llvm::SMLoc());
}

std::unique_ptr<PostriscOperand> PostriscAsmParser::defaultMemoryOrderingArgOp() const {
  return PostriscOperand::createMemoryOrderingArg(llvm::postrisc::RELAXED,
                                    llvm::SMLoc());
}
