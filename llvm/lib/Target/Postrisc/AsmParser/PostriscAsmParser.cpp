...
#include "MCTargetDesc/PostriscBaseInfo.h"
...
#include "PostriscGenAsmMatcher.inc"
  ...
  ParseStatus parseMemoryOrderingArg(OperandVector &Operands);

  bool parseInstruction(ParseInstructionInfo &Info, StringRef Name,
                        SMLoc NameLoc, OperandVector &Operands) override;
  ParseStatus parseDirective(AsmToken DirectiveID) override;

  std::unique_ptr<PostriscOperand> defaultFRMArgOp() const;
  std::unique_ptr<PostriscOperand> defaultMemoryOrderingArgOp() const;
...
private:
  ...
  struct FRMOp {
    PostriscFPRndMode::RoundingMode FRM;
  };

  struct MemoryOrderingOp {
    PostriscMemoryOrdering::MemoryOrdering  MO;
  };
...
public:
  ...
  bool isFRMArg() const { return Kind == k_FRM; }
  bool isMemoryOrderingArg() const { return Kind == k_MemoryOrdering; }
  ...
  PostriscFPRndMode::RoundingMode getFRM() const {
    assert(Kind == k_FRM && "Invalid type access!");
    return FRM.FRM;
  }

  PostriscMemoryOrdering::MemoryOrdering getMemoryOrdering() const {
    assert(Kind == k_MemoryOrdering && "Invalid type access!");
    return MO.MO;
  }
  ...
  void print(raw_ostream &OS, const MCAsmInfo &MAI) const override {
    ...
    case k_FRM:       OS << "frm: " << roundingModeToString(getFRM()) << "\n";  break;
    case k_MemoryOrdering:  OS << "frm: " << memoryOrderingToString(getMemoryOrdering()) << "\n";  break;
  ...
  static std::unique_ptr<PostriscOperand> CreateReg(MCRegister Reg, unsigned Kind,
                                                 SMLoc S, SMLoc E) {
  ...
  static std::unique_ptr<PostriscOperand>
  createFRMArg(PostriscFPRndMode::RoundingMode FRM, SMLoc S) {
    auto Op = std::make_unique<PostriscOperand>(k_FRM);
    Op->FRM.FRM = FRM;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  static std::unique_ptr<PostriscOperand>
  createMemoryOrderingArg(PostriscMemoryOrdering::MemoryOrdering MO, SMLoc S) {
    auto Op = std::make_unique<PostriscOperand>(k_MemoryOrdering);
    Op->MO.MO = MO;
    Op->StartLoc = S;
    Op->EndLoc = S;
    return Op;
  ...
  void addMemoryOrderingArgOperands(MCInst &Inst, unsigned N) const {
    assert(N == 1 && "Invalid number of operands!");
    Inst.addOperand(MCOperand::createImm(getMemoryOrdering()));
  }
...
ParseStatus PostriscAsmParser::parseFRMArg(OperandVector &Operands) {
  if (getLexer().isNot(AsmToken::Identifier))
    return TokError(
        "operand must be a valid floating point rounding mode mnemonic");

  StringRef Str = getLexer().getTok().getIdentifier();
  PostriscFPRndMode::RoundingMode FRM = PostriscFPRndMode::stringToRoundingMode(Str);

  if (FRM == PostriscFPRndMode::Invalid)
    return TokError(
        "operand must be a valid floating point rounding mode mnemonic");

  Operands.push_back(PostriscOperand::createFRMArg(FRM, getLoc()));
  Lex(); // Eat identifier token.
  return ParseStatus::Success;
}

ParseStatus PostriscAsmParser::parseMemoryOrderingArg(OperandVector &Operands) {
  if (getLexer().isNot(AsmToken::Identifier))
    return TokError(
        "operand must be a valid memory ordering mnemonic");

  StringRef Str = getLexer().getTok().getIdentifier();
  PostriscMemoryOrdering::MemoryOrdering MO = PostriscMemoryOrdering::stringToMemoryOrdering(Str);

  if (MO == PostriscMemoryOrdering::Invalid)
    return TokError(
        "operand must be a valid memory ordering mnemonic");

  Operands.push_back(PostriscOperand::createMemoryOrderingArg(MO, getLoc()));
  Lex(); // Eat identifier token.
  return ParseStatus::Success;
}

std::unique_ptr<PostriscOperand> PostriscAsmParser::defaultFRMArgOp() const {
  return PostriscOperand::createFRMArg(PostriscFPRndMode::RoundingMode::DYN,
                                    llvm::SMLoc());
}

std::unique_ptr<PostriscOperand> PostriscAsmParser::defaultMemoryOrderingArgOp() const {
  return PostriscOperand::createMemoryOrderingArg(PostriscMemoryOrdering::MemoryOrdering::RELAXED,
                                    llvm::SMLoc());
}
