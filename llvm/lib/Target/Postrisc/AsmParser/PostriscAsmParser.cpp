...
#include "PostriscGenAsmMatcher.inc"
  ...
  const PostriscMCExpr *adjustPICRelocation(PostriscMCExpr::VariantKind VK,
...
PostriscAsmParser::parsePostriscAsmOperand(std::unique_ptr<PostriscOperand> &Op,
  ...
  case AsmToken::Percent: {
    ...
    if (matchPostriscAsmModifiers(EVal, E)) {
      ...
      LLVM_DEBUG(dbgs() << "parsePostriscAsmOperand: Op=" << (*Op.get())
                        << " EVal=" << (*EVal) << "\n");
    }
    break;
  }
  ...
  case AsmToken::Identifier: {
    ...
    if (!getParser().parseIdentifier(Identifier)) {
      ...
      PostriscMCExpr::VariantKind Kind = PostriscMCExpr::VK_POSTRISC_NONE;

      if (getContext().getObjectFileInfo()->isPositionIndependent()) {
        if (isCall)
          Kind = PostriscMCExpr::VK_POSTRISC_JUMP;
        else
          Kind = PostriscMCExpr::VK_POSTRISC_PCREL_FWD;
      }

      Res = PostriscMCExpr::create(Kind, Res, getContext());
...
#define X(NAME) .Case(#NAME, Postrisc::NAME)
  RegNo = StringSwitch<unsigned>(name)
#define R(ID) X(r##ID)
...
#undef R
  ...
  RegNo = StringSwitch<unsigned>(name)
    X(ip) X(eip) X(fpcr) X(eca) X(rsc) X(bsp) X(rsp) X(bsp)
...
static bool hasGOTReference(const MCExpr *Expr) {
  switch (Expr->getKind()) {
  case MCExpr::Target:
    if (const PostriscMCExpr *SE = dyn_cast<PostriscMCExpr>(Expr))
...
PostriscAsmParser::adjustPICRelocation(PostriscMCExpr::VariantKind VK,
                                    const MCExpr *subExpr) {
  return PostriscMCExpr::create(VK, subExpr, getContext());
}
...
bool PostriscAsmParser::matchPostriscAsmModifiers(const MCExpr *&EVal,
  ...
  PostriscMCExpr::VariantKind VK = PostriscMCExpr::parseVariantKind(name);
  switch (VK) {
  case PostriscMCExpr::VK_POSTRISC_NONE:
    Error(getLoc(), "invalid operand modifier");
    return false;

  case PostriscMCExpr::VK_POSTRISC_TLS_GD_CALL:
    // These are special-cased at tablegen level.
    return false;

  default:
    LLVM_DEBUG(dbgs() << "matchPostriscAsmModifiers:VariantKind=" << (int)VK << "\n");
    break;
  }
...
extern "C" LLVM_EXTERNAL_VISIBILITY void LLVMInitializePostriscAsmParser() {
