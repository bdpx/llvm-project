...
u32 test_struct_memset(char *restrict s, size_t n, const char *restrict fmt, va_list ap)
{
    ...
    // CXHECK:    call.r %r3, %jpc(memset)
    // .LBB0_1:   ! %loadstoreloop
    //            ! =>This Inner Loop Header: Depth=1
    // CXHECK: st.b.xd %gz, %r3, %r1, 0, 0
    // CXHECK: addi %r1, %r1, 1
    // CXHECK: bdltui %r1, 232, .LBB0_1

    // CHECK: st.q %gz, %sp, 64
    // CHECK: st.q %gz, %sp, 80
    // CHECK: st.q %gz, %sp, 128
    // CHECK: st.q %gz, %sp, 144
    // CHECK: st.q %gz, %sp, 208
    // CHECK: st.d %gz, %sp, 224
    // CHECK: st.q %gz, %sp, 192
    // CHECK: st.q %gz, %sp, 176
    // CHECK: st.q %gz, %sp, 160
    // CHECK: st.q %gz, %sp, 112
    // CHECK: st.q %gz, %sp, 96
    // CHECK: st.q %gz, %sp, 48
    // CHECK: st.q %gz, %sp, 32
    // CHECK: st.q %gz, %sp, 16
    ...
    // CHECK: call.r %r2, %jpc(test_struct_memset_consumer)
