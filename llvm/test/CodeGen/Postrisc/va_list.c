...
int test_va_start(int count, ...)
{
  ...
  // CHECK: std    %r6, %sp, 8
  va_start(args, count);
  // CHECK: addiws %r3, %r1, 1
  // CHECK: addiws %r4, %r1, -1
  // CHECK: addi   %r5, %sp, 16
  // CHECK: callr  %r2, %jpc(test_va_list_callback)
...
int test_va_start_end(int count, ...)
{
  ...
  // CHECK:     callr %r3, %jpc(test_va_callback)
  // CHECK:     mov   %r1, %r4
  // CHECK:     retf  144
...
int test_printf_valist(char const *fmt, va_list ap)
{
  ...
  // CHECK: addi %r1, %sp, 0
  // CHECK: mov %r5, %r1
  // CHECK: callr %r3, %jpc(test_printf_callback)
...
int test_vprintf_valist(char const *fmt, va_list ap)
{
  ...
  // CHECK: ldi %r4, 100
  // CHECK: addi %r5, %sp, 16
  // CHECK: callr %r3, %jpc(test_vprintf_callback)
...
int test_vacopy(char const *fmt, va_list ap)
{
  ...
  // CHECK:     addi	%r1, %sp, 0
  // CHECK:     mov	%r4, %r1
  // CHECK:     callr	%r3, %jpc(test_valist_callback)
  // CHECK:     mov	%r4, %r1
  // CHECK:     callr	%r3, %jpc(test_valist_callback)
  // CHECK:     mov	%r1, %r4
  // CHECK:     retf	16
