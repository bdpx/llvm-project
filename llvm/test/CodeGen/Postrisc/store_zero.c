...
{
  // CHECK: st_i8 %gz, %r1, 4000
  ptr[4000] = 0;
  // CHECK: st_iprel_i8 %gz, %pcfwd(i8_zero)
  i8_zero = 0;
  // CHECK: st_xi64_i8 %gz, %r1, %r2, 0, 10
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i8 %gz, %r1, %r3, 0, 10
  ptr[ind32+10] = 0;
}
...
void test_store_based_u8_zero(u8 *ptr, i64 ind64, i32 ind32, u32 ind32u)
{
  // CHECK: st_i8 %gz, %r1, 4000
  ptr[4000] = 0;
  // CHECK: st_iprel_i8 %gz, %pcfwd(u8_zero)
  u8_zero = 0;
  // CHECK: st_xi64_i8 %gz, %r1, %r2, 0, 10
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i8 %gz, %r1, %r3, 0, 10
...
{
  // CHECK: st_i16 %gz, %r1, 8000
  ptr[4000] = 0;
  // CHECK: st_iprel_i16 %gz, %pcfwd(i16_zero)
  i16_zero = 0;
  // CHECK: st_xi64_i16 %gz, %r1, %r2, 1, 20
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i16 %gz, %r1, %r3, 1, 20
  ptr[ind32+10] = 0;
}
...
void test_store_based_u16_zero(u16 *ptr, i64 ind64, i32 ind32, u32 ind32u)
{
  // CHECK: st_i16 %gz, %r1, 8000
  ptr[4000] = 0;
  // CHECK: st_iprel_i16 %gz, %pcfwd(u16_zero)
  u16_zero = 0;
  // CHECK: st_xi64_i16 %gz, %r1, %r2, 1, 20
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i16 %gz, %r1, %r3, 1, 20
...
{
  // CHECK: st_i32 %gz, %r1, 16000
  ptr[4000] = 0;
  // CHECK: st_iprel_i32 %gz, %pcfwd(i32_zero)
  i32_zero = 0;
  // CHECK: st_xi64_i32 %gz, %r1, %r2, 2, 40
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i32 %gz, %r1, %r3, 2, 40
  ptr[ind32+10] = 0;
}
...
void test_store_based_u32_zero(u32 *ptr, i64 ind64, i32 ind32, u32 ind32u)
{
  // CHECK: st_i32 %gz, %r1, 16000
  ptr[4000] = 0;
  // CHECK: st_iprel_i32 %gz, %pcfwd(u32_zero)
  u32_zero = 0;
  // CHECK: st_xi64_i32 %gz, %r1, %r2, 2, 40
  ptr[ind64+10] = 0;
  // CHECK: st_xi32_i32 %gz, %r1, %r3, 2, 40
...
{
  // CHECK: st_i64 %gz, %r1, 32000
  ptr[4000] = 0;
  // CHECK: st_iprel_i64 %gz, %pcfwd(i64_zero)
  i64_zero = 0;
  // CHECK: st_xi64_i64 %gz, %r1, %r2, 3, 40
  ptr[ind64+5] = 0;
  // CHECK: st_xi32_i64 %gz, %r1, %r3, 3, 40
  ptr[ind32+5] = 0;
}
...
void test_store_based_u64_zero(u64 *ptr, i64 ind64, i32 ind32, u32 ind32u)
{
  // CHECK: st_i64 %gz, %r1, 32000
  ptr[4000] = 0;
  // CHECK: st_iprel_i64 %gz, %pcfwd(u64_zero)
  u64_zero = 0;
  // CHECK: st_xi64_i64 %gz, %r1, %r2, 3, 40
  ptr[ind64+5] = 0;
  // CHECK: st_xi32_i64 %gz, %r1, %r3, 3, 40
...
{
  // CHECK: st_i128 %gz, %r1, 64000
  ptr[4000] = 0;
  // CHECK: st_iprel_i128 %gz, %pcfwd(i128_zero)
  i128_zero = 0;
  // CHECK: st_xi64_i128 %gz, %r1, %r2, 4, 48
  ptr[ind64+3] = 0;
  // CHECK: st_xi32_i128 %gz, %r1, %r3, 4, 48
  ptr[ind32+3] = 0;
}
...
void test_store_based_u128_zero(u128 *ptr, i64 ind64, i32 ind32, u32 ind32u)
{
  // CHECK: st_i128 %gz, %r1, 64000
  ptr[4000] = 0;
  // CHECK: st_iprel_i128 %gz, %pcfwd(u128_zero)
  u128_zero = 0;
  // CHECK: st_xi64_i128 %gz, %r1, %r2, 4, 48
  ptr[ind64+3] = 0;
  // CHECK: st_xi32_i128 %gz, %r1, %r3, 4, 48
